...
if __name__ == '__main__':
    ...
    myLicense = Tp(LICENSE_TPL).substitute(
        projectDescription="xLR - Dynamic LR(1) Grammar Parser",
        projectName="xLR",
        moduleName="grammar",
        filename="${filename}",
    )

    GXLR = Generator(json_dir=JSON_DIR / "xLR",
                       template_dir=TEMPLATE_DIR / "xLR",
                       out_dir=OUT_DIR / "xLR",
                       target="GrammarEntry")
    GXLR.set_license(Tp(myLicense).substitute(filename="${filename}"))
    tokens = sorted(set(GXLR.tokens) | set(TERMINALS.keys()))
    GXLR.set_extend_tokens(tokens)
